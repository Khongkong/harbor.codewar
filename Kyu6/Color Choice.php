...
function checkchoose($designs, $colors) {
    for($colorChoices = 0; $colorChoices < $colors; $colorChoices++){
        $colorComination = round
            (
                factorial($colors) / (factorial($colorChoices) * factorial($colors - $colorChoices))
            );
        if($colorComination == $designs){
            return $colorChoices;
        }
    }
    return -1;
